...
def get_data(now_time, history_time, site_name):
    ...
    # hws_data.to_csv('./data/{}_original.csv'.format(site_name), index=False)
    ...
    # hws_data_csv = [hws_data['time'],hws_data['Rc']]
    # hws_data.to_csv('./data/{}_original.csv'.format(site_name), mode='a', index=False)
    # date = hws_data['time']
    # tp = hws_data['Rc']
    # with open(fr'original.csv','a',newline='') as fp:
    #     writer=csv.writer(fp,delimiter=',',quotechar='"',quoting=csv.QUOTE_MINIMAL)
    #     writer.writerow(['date','tp'])
    #     # for i in range(len(date)):
    #     writer.writerows([hws_data_csv
    #                      ])
...
def write_database(loaddata, end_time_unix,site_name):
    ...
    for i in range(loaddata.shape[1]):
        tmp_time = pd.to_datetime(end_time_unix + (i + 1) * time_freq * 60, unit='s').strftime('%Y-%m-%d %H:%M:%S')
...
def save_data(now_time, site_name):
    ...
    loaddata = np.load(
        './results/informer_JFNG_data_15min_ftMS_sl48_ll24_pl12_dm512_nh8_el2_dl1_df2048_atprob_fc5_ebtimeF_dtTrue_mxTrue_test_0/real_prediction_{}.npy'.format(
            site_name))
    ...
    #plt.plot(loaddata[0, :, -1], label='Prediction')
    #plt.grid(linestyle='--', linewidth=1, alpha=0.5)
    #plt.xlabel('sample')
    #plt.ylabel('tp/mm')
    #plt.legend()
    #plt.savefig(figure_path + 'prediction_picture_{0}_{1}.png'.format(site_name,now_time))
    #plt.close()
...
if __name__ == '__main__':
    ...
    for i in range(0, 500):
        x = start_time + datetime.timedelta(hours=3*i)
        #print(x.timestamp())
